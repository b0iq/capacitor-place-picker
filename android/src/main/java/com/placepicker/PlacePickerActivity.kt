...
class PlacePickerActivity : AppCompatActivity(), OnMapReadyCallback,
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    options = Json.decodeFromString(PlacePickerState.call?.data.toString())
...
//    mMap.setPadding(10, (supportActionBar?.height ?: 0), 10, getActionBarHeight())
  ...
  fun resolve() {
    ...
    val m = mapper.convertValue(PlacePickerState.result, Map::class.java);
    PlacePickerState.call?.resolve(JSObject.fromJSONObject(JSONObject(m)))
  }
  override fun finish() {
    super.finish()
    if (this.isDone) {
      ...
      if (PlacePickerState.options?.rejectOnCancel == true) {
        PlacePickerState.call?.reject("Canceled")
      } else {
