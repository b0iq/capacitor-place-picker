...
class PlacePickerViewController: UIViewController {
    ...
    // MARK: - Inits
    init(_ call: CAPPluginCall, _ options: PlacePickerOptions) {
        self.call = call
    ...
    override func viewDidDisappear(_ animated: Bool) {
        if (call != nil) {
            call?.reject("dismissed", "Modal closed by user", NSError(domain: "pickerView", code: 11), nil)
            call = nil
        }
    }
    ...
    @objc private func closePicker() {
        if (options.rejectOnCancel) {
            if (call != nil) {
                call?.reject("cancel", "User cancel the operation and `rejectOnCancel` is enabled", NSError(domain: "pickerView", code: 13), nil)
                call = nil
        ...
        call?.resolve(try! PlacePickerResult(
